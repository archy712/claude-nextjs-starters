...
- **Task 007: 부가 기능 및 사용자 경험 향상**
  ...
  - 파일 업로드 및 미디어 처리
  - Playwright MCP를 활용한 신규 기능 E2E 테스트 수행

- **Task 008: 성능 최적화 및 배포**
  - 성능 최적화 및 캐싱 전략 구현
  - 테스트 코드 작성 및 CI/CD 파이프라인 구축
  - 모니터링 및 로깅 시스템 구성
  - Playwright MCP로 회귀 테스트(regression test) 수행 후 배포
```
...
5. **테스트 필수 (Phase 무관)**: API 연동 또는 비즈니스 로직을 포함하는 Task는 소속 Phase와 관계없이 반드시 아래를 만족해야 함
   - 구현 사항 목록 마지막에 "Playwright MCP를 활용한 테스트 수행" 항목 명시
   - 정상 케이스뿐 아니라 실패/에러 케이스, 엣지 케이스까지 검증 대상에 포함
   - 구현만 완료하고 테스트를 생략한 상태는 "완료(✅)"로 표시할 수 없음
