...
- 수락 기준과 완료 조건 정의
- **API 연동 및 비즈니스 로직 구현 시 Playwright MCP를 활용한 테스트 필수 (예외 없음)**
- 테스트 시나리오는 정상 플로우만이 아니라 에러/예외 케이스, 엣지 케이스, 기존 기능 회귀 영향까지 꼼꼼하게 포함
- **구현이 끝났다고 완료가 아님 — Playwright MCP로 테스트를 실제 실행하고 통과를 확인해야만 해당 구현 단계가 완료됨**
- 테스트 실패 시 다음 단계로 진행하지 않고, 원인을 수정한 뒤 통과할 때까지 재테스트
...
- **Task 005: 데이터베이스 및 API 개발** - 우선순위
  ...
  - Playwright MCP를 활용한 API 엔드포인트 통합 테스트 (정상/에러/엣지 케이스 모두 포함)
  - 테스트 통과 확인 후에만 완료(✅) 처리
...
- **Task 006: 인증 및 권한 시스템 구현**
  ...
  - 권한 기반 접근 제어 구현
  - 보안 미들웨어 및 세션 관리
  - Playwright MCP로 인증 플로우 E2E 테스트 수행 (성공/실패/권한 없음 케이스 포함)
  - 테스트 통과 확인 후에만 완료(✅) 처리
...
- **Task 006-1: 핵심 기능 통합 테스트**
  ...
  - API 연동 및 비즈니스 로직 검증
  - 에러 핸들링 및 엣지 케이스 테스트
  - 회귀 테스트: 이전 Task에서 구현한 기능이 이번 변경으로 깨지지 않았는지 확인
  - 하나라도 실패 시 해당 Task로 돌아가 수정 후 전체 재테스트
...
- [ ] API 연동 및 비즈니스 로직 구현 Task에 Playwright MCP 테스트가 예외 없이 포함되었는가?
- [ ] 각 작업 파일에 "## 테스트 체크리스트" 섹션이 명시되었는가?
- [ ] 테스트 체크리스트가 정상 플로우 · 에러/예외 케이스 · 엣지 케이스 · 회귀 테스트 4개 카테고리를 모두 다루는가?
- [ ] 모든 사용자 플로우에 대한 E2E 테스트 시나리오가 정의되었는가?
- [ ] "테스트 통과 전에는 완료(✅) 처리하지 않는다"는 게이팅 규칙이 워크플로우에 명시되었는가?
