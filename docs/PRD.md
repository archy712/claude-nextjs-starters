...
노션 프로퍼티 이름은 코드(`src/types/invoice.ts`의 `NotionInvoiceProperties`)와 1:1로 매핑되도록 영문 snake_case로 통일되어 있다.
...
├── issue_date (Date)
├── valid_until (Date)
├── status (Status: 대기/승인/거절) — 일반 Select가 아닌 노션 내장 Status 타입
├── total_amount (Number)
...
├── quantity (Number)
├── unit_price (Number)
├── amount (Formula: quantity × unit_price)
